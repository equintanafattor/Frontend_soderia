...
extension RolX on Rol { String get label => switch (this) {
  Rol.admin => 'Admin', Rol.ventas => 'Ventas', Rol.repartidor => 'Repartidor', Rol.caja => 'Caja'
}; }
...
class _UsuarioAddScreenState extends State<UsuarioAddScreen> {
  ...
  bool get _isValid => _formKey.currentState?.validate()==true && _rol!=null;

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text('Agregar usuario'),
        actions: [IconButton(icon: const Icon(Icons.check), onPressed: _isValid? _submit : null)],
      ),
      body: Form(
        key: _formKey,
        ...
        child: ListView(
          ...
          children: [
            ...
            _tf('Email', _mail, keyboard: TextInputType.emailAddress, validator: _mailOk),
            ...
            Wrap(
              spacing: 10,
              children: Rol.values.map((r) => ChoiceChip(
                label: Text(r.label),
                selected: _rol==r,
            ...
            Row(
              children: [
                Switch.adaptive(value: _activo, onChanged: (v)=> setState(()=> _activo=v)),
                const Text('Activo'),
                const Spacer(),
                OutlinedButton.icon(
  ...
  Widget _tf(String label, TextEditingController c, {String? Function(String?)? validator, TextInputType? keyboard}) =>
      TextFormField(controller: c, decoration: InputDecoration(labelText: label), validator: validator, keyboardType: keyboard);

  String? _req(String? v)=> (v==null||v.trim().isEmpty) ? 'Obligatorio' : null;
  String? _mailOk(String? v){
    if (_req(v)!=null) return 'Obligatorio';
    return RegExp(r'^[^\s@]+@[^\s@]+\.[^\s@]+$').hasMatch(v!) ? null : 'Email inválido';
  }
